...
jsonObject ::
  Parser Assoc
jsonObject = betweenSepbyComma '{' '}' assocParser
...
--   ||| (jsonString >>= (\s -> valueParser (JsonString s)))
--   ||| (jsonArray >>= (\a -> valueParser (JsonArray a)))
--   ||| (jsonObject >>= (\o -> valueParser (JsonObject o)))
--   ||| (jsonNumber >>= (\n -> valueParser (JsonRational n)))
...
readJsonValue path = readFile path >>= (return . parse jsonValue)
