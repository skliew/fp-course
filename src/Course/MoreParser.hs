...
hex ::
  Parser Char
-- hex = thisMany 4 (satisfy isHexDigit) >>= (\h -> case readHex h of
--                                                   Empty -> constantParser (UnexpectedString h)
--                                                   Full a -> valueParser (chr a))
hex = chr . hInt <$> thisMany 4 (satisfy isHexDigit)
        where hInt h = readHex h ?? 0
