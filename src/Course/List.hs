...
reverse ::
  List a
  -> List a
reverse x = reverseAcc x Nil

reverseAcc :: List a -> List a -> List a
reverseAcc Nil acc = acc
reverseAcc (h :. t) acc = let b' = (h :. acc)
                          in reverseAcc t b'

-- This is sloooww
-- reverse Nil = Nil
-- reverse (x :. xs) = let reversed = (reverse xs)
--                         result = reversed `seq` (reversed ++ (x :. Nil))
...
-- Note:
-- I missed to look at the helper functions. Could have implemented some of the functions using
-- those functions.
-- Also, if we implement `reverse` with `(++)`, it is going to be slow possibly because we're building
-- up thunks.
