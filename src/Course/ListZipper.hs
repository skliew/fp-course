...
findLeft f (ListZipper l a r) = case break f l of
                                  (_, Nil) -> IsNotZ
                                  (l', (x:.xs)) -> IsZ (ListZipper xs x ((reverse l') ++ (a:.r)))
...
findRight ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findRight f (ListZipper l a r) = case break f r of
                                  (_, Nil) -> IsNotZ
                                  (l', (x:.xs)) -> IsZ (ListZipper ((reverse l') ++ (a:.l)) x xs)
...
moveLeft (ListZipper (x:.xs) a r) = IsZ (ListZipper xs x (a:.r))
...
moveRight (ListZipper l a (x:.xs)) = IsZ (ListZipper (a:.l) x xs)
...
moveLeftN ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
moveLeftN n lz | n == 0 = IsZ lz
moveLeftN n lz | n < 0 = moveRightN (negate n) lz
moveLeftN n lz = moveLeftN (n - 1) -<< moveLeft lz
...
moveRightN ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
moveRightN n lz | n == 0 = IsZ lz
moveRightN n lz | n < 0 = moveLeftN (negate n) lz
moveRightN n lz = moveRightN (n - 1) -<< moveRight lz
...
moveLeftN' n lz@(ListZipper l _ r) = case moveLeftN n lz of
...
moveRightN' ::
  ...
  -> ListZipper a
  -> Either Int (ListZipper a)
moveRightN' n lz@(ListZipper l _ r) = case moveRightN n lz of
                       IsNotZ -> Left (length (if n < 0 then l else r))
                       IsZ lz' -> Right lz'
...
-- end (ListZipper l a r) = let (x:.xs) = reverse r
--                          in ListZipper (xs ++ a :. l) x Nil
...
-- /Tip:/ Use `zipWith`
  (<*>) (ListZipper fl fa fr) (ListZipper l a r) = ListZipper (zipWith ($) fl l) (fa a) (zipWith ($) fr r)
...
instance Extend ListZipper where
  (<<=) f a = ListZipper fl (f a) fr
    where fl = unfoldr ((<$>) (\z -> (f z, z)) . toOptional . moveLeft)  a
          fr = unfoldr ((<$>) (\z -> (f z, z)) . toOptional . moveRight)  a
...
instance Traversable MaybeListZipper where
  ...
  traverse f (IsZ a) = IsZ <$> traverse f a
